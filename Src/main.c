...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    delay_ms(10);
    LCD_ShowString(72, LCD_H / 2 - 20, (uint8_t *)"Networking...", WHITE, BLACK, 24, 0); // 12*6,16*8,24*12,32*16
    ...
    if (air_init())
    {
        EMlog(LOG_ERROR, "air init error\n");
    }
    ...
    // ai init
    // AI_Init();
    EMlog(LOG_INFO, "init successful\n");

    // double ax, ay, az = 0;
    // float pitch, roll, yaw = 0;
    // while (1) // test
    // {
    //     // max30102_cal_data(); // 得心率血氧数
    //     // g_lsm6ds3.getacc(&ax, &ay, &az); // 得陀螺仪
    //     // EMlog(LOG_DEBUG, "x:%f, y:%f, z:%f\r\n", ax, ay, az);
